...
const itemVariants = { hidden: { opacity: 0, y: 20, scale: 0.95 }, visible: { opacity: 1, y: 0, scale: 1, transition: { type: "spring", stiffness: 100, damping: 20 } } }
  ...
  return (
    <section id="services" className="w-full py-24 relative z-10">
      <div className="container px-4 md:px-6 mx-auto">
        ...
        <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto">
          {services.map((service, i) => (
            <motion.div variants={itemVariants} key={i} className="glass-card rounded-2xl p-7 flex flex-col gap-4 group hover:-translate-y-1 hover:border-[#6E8CFB]/25 transition-all duration-500 hover:shadow-[0_8px_30px_rgba(110,140,251,0.1)] cursor-default">
